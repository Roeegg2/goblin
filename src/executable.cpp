...
allocate_new:
    ...
    dtvs[tid][ti->ti_module - 1] =
        mmap(nullptr, s_tls.m_total_imgs_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);

    std::memcpy(dtvs[tid][ti->ti_module - 1], s_tls.m_init_imgs[ti->ti_module - 1].m_data,
                s_tls.m_init_imgs[ti->ti_module - 1].m_size);
...
void Executable::init_thread_static_tls() {
    void *tp = mmap(nullptr, s_tls.m_total_imgs_size + sizeof(struct tcb), PROT_READ | PROT_WRITE,
                    MAP_PRIVATE | MAP_ANONYMOUS, -1,
                    0); // allocate memory for the tls blocks and tcb, as
                        // specified in variant 2
    tp = reinterpret_cast<void *>(reinterpret_cast<Elf64_Addr>(tp) +
                                  s_tls.m_total_imgs_size); // we want to point to the end of the tls
    ...
    tp = reinterpret_cast<void *>(reinterpret_cast<Elf64_Addr>(tp) -
                                  s_tls.m_total_imgs_size); // point to the start of the tls block images
                                                            // again
    for (auto &img : s_tls.m_init_imgs) {                   // for each TLS block image
        if (img.m_is_static_model) {                        // if the TLS block is using static model
            ...
            dtvs[ctp->tid].push_back(tp); // NOTE: we initlized the m_init_imgs vector the same order we
