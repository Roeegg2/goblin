...
namespace Goblin {
    ...
    void ELF_File::parse_prog_headers(void) {
        ...
        for (uint16_t i = 0; i < m_elf_header.e_phnum; i++) {
            ...
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_type), 4); // segment type
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_flags), 4); // segment flags
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_offset), 8); // offset in file
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_vaddr), 8); // virtual address in memory
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_paddr), 8); // physical address in memory
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_filesz), 8); // size of segment in file
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_memsz), 8); // size of segment in memory
            m_elf_file.read(reinterpret_cast<char*>(&m_prog_headers[i].p_align), 8); // alignment
        }
    }
    ...
    void ELF_File::parse_sect_headers(void) {
        ...
        for (uint16_t i = 0; i < m_elf_header.e_shnum; i++) {
            ...
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_name), 4); // offset into the .shstrtab section
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_type), 4); // type of section
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_flags), 8); // section attributes
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_addr), 8); // virtual address in memory
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_offset), 8); // offset in file
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_size), 8); // size of section
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_link), 4); // index of a related section
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_info), 4); // depends on section type
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_addralign), 8); // alignment
            m_elf_file.read(reinterpret_cast<char*>(&m_sect_headers[i].sh_entsize), 8); // size of each entry if section holds a table
        }
    }
