...
namespace Goblin {
    ...
    void ELF_File::print_prog_headers() const {
        ...
        for (uint8_t i = 0; i < m_elf_header.e_phnum; i++) {
            std::cout << "Type: ";
            print_ph_type(i);
            std::cout << ", Flags: 0x" << std::hex << m_prog_headers[i].p_flags
                      << ", Offset: 0x" << m_prog_headers[i].p_offset
                      << ", Virtual address: 0x" << m_prog_headers[i].p_vaddr
                      << ", Physical address: 0x" << m_prog_headers[i].p_paddr
                      << ", File size: 0x" << m_prog_headers[i].p_filesz
                      << ", Memory size: 0x" << m_prog_headers[i].p_memsz
                      << ", Alignment: 0x" << m_prog_headers[i].p_align << "\n";
        }
    }

    void ELF_File::print_sh_type(uint16_t i) const {
        static const std::vector<std::string> c_sh_types = {
    ...
    void ELF_File::print_sect_headers() const {
        ...
        for (uint8_t i = 0; i < m_elf_header.e_shnum; i++) {
            ...
            std::cout << ", Type: ";
            print_sh_type(i);
            std::cout << ", Flags: 0x" << std::hex << m_sect_headers[i].sh_flags
                      << ", Address: 0x" << m_sect_headers[i].sh_addr
                      << ", Offset: 0x" << m_sect_headers[i].sh_offset
                      << ", Size: 0x" << m_sect_headers[i].sh_size
                      << ", Link: 0x" << m_sect_headers[i].sh_link
                      << ", Info: 0x" << m_sect_headers[i].sh_info
                      << ", Address alignment: 0x" << m_sect_headers[i].sh_entsize
